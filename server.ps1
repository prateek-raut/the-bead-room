    ...
    while ($listener.IsListening) {
        ...
        if ($urlPath -eq "/api/admin/login" -and $httpMethod -eq "POST") {
            ...
            try {
                ...
                $auth = $db.adminAuth
                if (-not $auth) {
                    $auth = [PSCustomObject]@{ email = "sarakamdar26@gmail.com"; password = "Pallas@123" }
                }

                if ($loginReq.email -and $loginReq.password -and ($loginReq.email.Trim().ToLower() -eq $auth.email.Trim().ToLower()) -and ($loginReq.password -eq $auth.password)) {
                    $token = "beadroom-admin-" + [System.Guid]::NewGuid().ToString()
                    $resObj = @{ success = $true; message = "Login successful"; token = $token; email = $auth.email }
                    $responseBytes = [System.Text.Encoding]::UTF8.GetBytes((ConvertTo-Json $resObj))
                    $response.ContentType = "application/json; charset=utf-8"
                    $response.StatusCode = 200
                    Write-Host "[ADMIN LOGIN SUCCESS] Logged in as $($auth.email)" -ForegroundColor Green
                } else {
                    $resObj = @{ success = $false; error = "Invalid admin email or password" }
                    $responseBytes = [System.Text.Encoding]::UTF8.GetBytes((ConvertTo-Json $resObj))
                    $response.ContentType = "application/json; charset=utf-8"
                    $response.StatusCode = 401
                    Write-Host "[ADMIN LOGIN FAILED] Invalid credentials provided" -ForegroundColor Red
                }
            } catch {
                $resObj = @{ success = $false; error = "Malformed login request" }
                $responseBytes = [System.Text.Encoding]::UTF8.GetBytes((ConvertTo-Json $resObj))
                $response.ContentType = "application/json; charset=utf-8"
        ...
        if ($urlPath -eq "/api/admin/change-password" -and $httpMethod -eq "POST") {
            ...
            try {
                ...
                if ($passReq.currentPassword -eq $auth.password) {
                    $auth.password = $passReq.newPassword
                    $db.adminAuth = $auth
                    Save-Database $db
                    $resObj = @{ success = $true; message = "Password updated successfully!" }
                    $responseBytes = [System.Text.Encoding]::UTF8.GetBytes((ConvertTo-Json $resObj))
                    $response.ContentType = "application/json; charset=utf-8"
                    $response.StatusCode = 200
                    Write-Host "[ADMIN PASSWORD CHANGED] Updated password for $($auth.email)" -ForegroundColor Green
                } else {
                    $resObj = @{ success = $false; error = "Current password is incorrect" }
                    $responseBytes = [System.Text.Encoding]::UTF8.GetBytes((ConvertTo-Json $resObj))
                    $response.ContentType = "application/json; charset=utf-8"
                    $response.StatusCode = 400
                }
            } catch {
                $resObj = @{ success = $false; error = "Malformed request" }
                $responseBytes = [System.Text.Encoding]::UTF8.GetBytes((ConvertTo-Json $resObj))
                $response.ContentType = "application/json; charset=utf-8"
                $response.StatusCode = 400
            }
