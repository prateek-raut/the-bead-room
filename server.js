...
app.post('/api/admin/login', (req, res) => {
  ...
  const auth = db.adminAuth || { email: 'sarakamdar26@gmail.com', password: 'Pallas@123' };
  const { email, password } = req.body;

  if (email && password && email.trim().toLowerCase() === auth.email.trim().toLowerCase() && password === auth.password) {
    const token = `beadroom-admin-${Date.now()}-${Math.round(Math.random() * 1e6)}`;
    res.json({ success: true, message: 'Login successful', token, email: auth.email });
  } else {
