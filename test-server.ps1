...
# 7. Test POST /api/admin/login
try {
    $loginPayload = @{ email = "sarakamdar26@gmail.com"; password = "Pallas@123" } | ConvertTo-Json
    $resLogin = Invoke-RestMethod -Uri "$baseUrl/api/admin/login" -Method Post -Body $loginPayload -ContentType "application/json"
    Write-Host "[PASS] POST /api/admin/login successful! Token: $($resLogin.token)" -ForegroundColor Green
} catch {
    Write-Host "[FAIL] POST /api/admin/login failed: $_" -ForegroundColor Red
}
